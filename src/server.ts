...
function rebuildDoors(
  registry: Registry,
  doors: Map<string, AgentDoor>,
  doorMiddlewares: Map<string, ReturnType<AgentDoor['middleware']>>,
  slugPatterns: Map<string, RegExp>,
): void {
  ...
  for (const row of stored) {
    try {
      ...
      const door = AgentDoor.fromOpenAPI(spec as unknown as Parameters<typeof AgentDoor.fromOpenAPI>[0], row.apiUrl, {
  ...
  if (loaded > 0 || failed.length > 0) {
    console.log(
      `Rebuilt ${loaded} door(s) from database` +
